...
Maintain cherry-pickable feature patches: each commit carries one complete feature
or bug fix, including its tests, generated artifacts, and required guidance. Fold
corrections into the owning commit and replay its descendants; do not append
follow-up or integration commits for the same feature. Keep independent changes
in separate commits.

Preserve upstream ancestry. A fix to upstream-owned behavior may remain a
...
- Bind provider state to the account captured for each outbound request. `ModelResolver`
  ...
  retaining readable history. Prompt-cache keys combine Session lineage with that scope.
  Late HTTP request hooks make identity unknown, so those requests omit opaque history
  and cache keys. Provider model-request hooks must use their supplied credential snapshot,
  rather than reselecting an account after model resolution.
...
- Keep the built-in environment heading distinct from Claude Code's preamble.
