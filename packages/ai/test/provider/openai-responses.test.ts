  ...
  it.effect("advertises a grammar tool as a freeform custom tool and replays its calls as raw text", () =>
    Effect.gen(function* () {
      ...
      const messages = [
        Message.user("Fix it."),
        Message.assistant([
          ToolCallPart.make({ id: "call_1", name: "apply_patch", input: { patchText: "*** Begin Patch\n*** End Patch" } }),
        ]),
        Message.tool({ id: "call_1", name: "apply_patch", result: "Success. Updated the following files:", resultType: "text" }),
        ...
        Effect.provide(
          fixedResponse(
            sseEvents(
              ...
              { type: "response.custom_tool_call_input.delta", output_index: 0, item_id: "ctc_1", delta: "*** End Patch" },
              {
