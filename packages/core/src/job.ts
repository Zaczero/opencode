...
export const make = Effect.gen(function* () {
  ...
  const settle = Effect.fnUntraced(function* (id: string, scope: Scope.Closeable, exit: Exit.Exit<string, unknown>) {
    ...
    const result = yield* SynchronizedRef.modifyEffect(
      ...
      Effect.fnUntraced(function* (registry): Effect.fn.Return<readonly [FinishResult, Registry]> {
        ...
        if (job.scope !== scope) return [{}, registry]
      ...
      Effect.gen(function* () {
        ...
        const result = yield* SynchronizedRef.modifyEffect(
          ...
          Effect.fnUntraced(function* (registry): Effect.fn.Return<readonly [StartResult, Registry]> {
            ...
            return [{ info: snapshot(job), scope }, setActive(registry, id, job)]
          }),
        )
        if ("scope" in result)
  ...
  const awaitOwned: Interface["awaitOwned"] = Effect.fn("Job.awaitOwned")(function* (sessionID) {
    ...
    const waits = [
      ...Array.fromIterable(registry.active.values())
  ...
  const removeSession: Interface["removeSession"] = Effect.fn("Job.removeSession")(function* (sessionID) {
    const completed_at = yield* Clock.currentTimeMillis
    const scopes = yield* SynchronizedRef.modifyEffect(state.jobs, (registry) =>
      Effect.gen(function* () {
        const active = new Map(registry.active)
        const terminal = new Map(registry.terminal)
        const scopes: Scope.Closeable[] = []
        for (const entries of [active, terminal]) {
        ...
        return [scopes, { active, terminal }] as const
      }),
    )
