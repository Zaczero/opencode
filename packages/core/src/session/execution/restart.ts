  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const recoverSubagent = Effect.fnUntraced(function* (
        ...
        const persistedTerminal = Effect.fnUntraced(function* () {
          if (!child.time.idle || !child.outcome) return undefined
          if (background.started_at !== undefined && DateTime.toEpochMillis(child.time.idle) < background.started_at)
            return undefined
          if (child.outcome === "interrupted") return { status: "cancelled" as const }
          if (child.outcome === "failed") return { status: "error" as const, error: "Subagent failed" }
          const messages = yield* store.context(recovery.childSessionID).pipe(Effect.orDie)
        ...
        const terminal = yield* persistedTerminal()
        if (terminal) {
          yield* notify(terminal)
          return
        }
