...
import { Cause, Context, Deferred, Effect, Exit, Fiber, Layer, Ref, Schema, SchemaGetter, Scope, Stream } from "effect"
...
const itWithoutPersistence = testEffect(
  AppNodeBuilder.build(LayerNode.group([Database.node, Bus.node]), [
    Bus.node.replace(Bus.configured({ persist: false })),
  ]),
)
...
describe("Bus", () => {
  ...
  it.effect("completes subscriptions when the Bus scope shuts down", () =>
    Effect.gen(function* () {
      ...
      const context = yield* Layer.buildWithScope(
        AppNodeBuilder.build(LayerNode.group([Database.node, Bus.node]), [Bus.node.replace(Bus.configured())]),
        scope,
      )
  ...
  it.effect("gates selected listeners before constructing them and preserves registration order", () =>
    Effect.gen(function* () {
      const bus = yield* Bus.Service
      const received = new Array<string>()
      yield* bus.listen(() => Effect.sync(() => received.push("filtered")), { types: new Set([SyncMessage.type]) })
  ...
  it.effect("unsubscribes selected listeners", () =>
    Effect.gen(function* () {
      const bus = yield* Bus.Service
      const received = new Array<string>()
      const unsubscribe = yield* bus.listen(() => Effect.sync(() => received.push("filtered")), {
