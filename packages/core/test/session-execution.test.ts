...
describe("SessionRestart background recovery", () => {
  ...
  it.effect("resumes a background subagent and notifies its parent exactly once", () =>
    Effect.gen(function* () {
      ...
      yield* database.db
        .update(SessionTable)
        .set({ idle_outcome: "succeeded", time_idle: running.started_at - 1 })
  ...
  it.effect("delivers a child terminal persisted before its running marker settled without rerunning it", () =>
    Effect.gen(function* () {
      const database = yield* Database.Service
      const jobs = yield* Job.Service
      const parent = Session.ID.make("ses_subagent_terminal_parent")
      const child = Session.ID.make("ses_subagent_terminal_child")
      yield* seedSessions(database, [parent])
      yield* seedSessions(database, [child], { parent_id: parent })
      const running = yield* jobs.start({
        id: child,
      ...
      expect(drained).toEqual([parent])
      expect(yield* SessionInbox.list(database.db, parent)).toMatchObject([
        { payload: { metadata: { source: "subagent", childID: child, state: "completed" } } },
      ])
      expect(yield* restarted.pendingBackground).toEqual([])
    }),
  )
