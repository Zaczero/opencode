...
describe("SessionHistory", () => {
  it.effect("reuses unchanged message objects while following durable revisions", () =>
    Effect.gen(function* () {
      ...
      const initial = yield* SessionHistory.load(db, sessionID, "latest", cache)
      const unchanged = yield* SessionHistory.load(db, sessionID, "latest", cache)
      ...
      const changed = yield* SessionHistory.load(db, sessionID, "latest", cache)
      ...
      const compacted = yield* SessionHistory.load(db, sessionID, "latest", cache)
      ...
      const resumedHistory = yield* SessionHistory.load(db, sessionID, "latest", cache)
      ...
      expect((yield* SessionHistory.load(db, sessionID, "latest", cache)).map((message) => message.id)).toEqual([
        compaction.id,
      ])

      cache.close()
      const afterClose = yield* SessionHistory.load(db, sessionID, "latest", cache)
      expect(afterClose[0]).not.toBe(compacted[0])
      const cold = SessionHistory.makeCache()
      const coldHistory = yield* SessionHistory.load(db, sessionID, "latest", cold)
  ...
  it.effect("does not cache storage without a durable revision", () =>
    Effect.gen(function* () {
      ...
      const updatedHistory = yield* SessionHistory.load(db, sessionID, "latest", cache)
  ...
  it.effect("keeps active cache scopes independent and releases one at close", () =>
    Effect.gen(function* () {
      ...
      const firstHistory = yield* SessionHistory.load(db, sessionID, "latest", firstCache)
      const secondHistory = yield* SessionHistory.load(db, otherSessionID, "latest", secondCache)
      yield* SessionHistory.load(db, sessionID, "latest", firstCache)
      yield* SessionHistory.load(db, otherSessionID, "latest", secondCache)

      firstCache.close()
      expect((yield* SessionHistory.load(db, sessionID, "latest", firstCache))[0]).not.toBe(firstHistory[0])
      expect((yield* SessionHistory.load(db, otherSessionID, "latest", secondCache))[0]).toBe(secondHistory[0])
