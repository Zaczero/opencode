    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.acquireRelease(Effect.sync(() => activeSubagents.add(running.id)), () =>
          ...
          const ordered = (yield* sessions.list({ parentID: parent.id })).data.filter((child) => child.id !== running.id)
