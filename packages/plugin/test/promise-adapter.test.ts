    ...
    Effect.scoped(
      fromPromise(
        define({
          ...
          setup: async (context) => {
            for await (const event of context.event.subscribe({ types: ["session.synthetic"] })) received.push(event)
          },
        }),
      ).effect(host),
    ),
...
test("Promise plugins can snapshot executing sessions and unfinished subagents", async () => {
  const executing = [{ sessionID: "ses_running", startedAt: 123 }]
  const subagents = [{ sessionID: "ses_child", startedAt: 100 }]
  const host = createHost(Stream.empty, Effect.succeed(executing), Effect.succeed(subagents))
  const received: unknown[] = []
    ...
    Effect.scoped(
      fromPromise(
        define({
          ...
          setup: async (context) =>
            void received.push(await context.session.executing(), await context.session.subagents()),
        }),
      ).effect(host),
    ),
  )

  expect(received).toEqual([executing, subagents])
})
...
function createHost(
  ...
  subagents: Context["session"]["subagents"] = Effect.die("unused session.subagents"),
) {
  const unused = (..._args: never[]) => Effect.die("unused")
  return {
    app: { name: "test", version: "test", channel: "test" },
    options: {},
    experimental: { terminal: { read: unused } },
    rpc: { register: unused, client: unused, list: unused },
    worktree: { list: unused, create: unused, remove: unused, refresh: unused, transform: unused, reload: unused },
    agent: { get: unused, list: unused, transform: unused, reload: unused },
    aisdk: { hook: unused },
    model: { list: unused, default: unused, transform: unused, reload: unused },
    provider: { list: unused, get: unused, transform: unused, reload: unused },
